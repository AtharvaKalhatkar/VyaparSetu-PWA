...
export function OrdersPage({ onNavigate }: { onNavigate: (p: string) => void })   {
  ...
  const filtered = allOrders.filter(i => i.invoiceNo.toLowerCase().includes(search.toLowerCase()) || i.partyName.toLowerCase().includes(search.toLowerCase()))
  ...
  const resetForm = () => {
    setShowForm(false); setLines([]); setPartyId(''); setDate(todayISO()); setNotes('')
  }
  ...
  const handleSave = () => {
    ...
    DB.invoices.save({
      id: generateId(), docType, invoiceNo: nextInvoiceNo(DB.invoices.list().filter(i => i.docType === docType).map(i => i.invoiceNo), 'ORD'),
      ...
      orderStatus: 'OPEN', notes,
    })
    resetForm()
  }

  const handleConvert = (order: Invoice) => {
    const isSale = order.type === 'SALE' || order.docType === 'SALE_ORDER'
    const subtotal = order.items.reduce((s, i) => s + i.amount, 0)
    const taxAmt = order.items.reduce((s, i) => s + i.amount * i.gstRate / 100, 0)
    const grandTotal = subtotal + taxAmt
    const dt = isSale ? 'SALE' as const : 'PURCHASE' as const
    const prefix = isSale ? 'INV' : 'PUR'
    const newInv = {
      id: generateId(), docType: dt, invoiceNo: nextInvoiceNo(DB.invoices.list().filter(i => i.docType === dt).map(i => i.invoiceNo), prefix),
      partyId: order.partyId, partyName: order.partyName, type: dt,
    ...
    DB.invoices.save({ ...order, orderStatus: 'CONVERTED', convertedTo: newInv.id })
    applyStockChanges(
      order.items.map(i => ({ itemId: i.itemId, quantity: i.quantity, unit: i.unit })),
      isSale ? 'SALE' : 'PURCHASE', false
    )
    const ledgType = isSale ? 'SALE' : 'PURCHASE'
    createLedgerEntry(newInv.partyId, newInv.partyName, ledgType, grandTotal, 'CREDIT', newInv.invoiceNo, 'Converted from order ' + order.invoiceNo, todayISO())
  }
  ...
  if (showForm) {
    return (
      <div style={{ padding: Spacing.lg, paddingBottom: 80 }}>
        <div style={{ fontSize: 18, fontWeight: 700, marginBottom: Spacing.md }}>New Order</div>
        <div style={s.toggleGroup}>
          <button onClick={() => setFormType('SALE')} style={s.toggle(formType === 'SALE', Colors.primary)}>Sale Order</button>
          <button onClick={() => setFormType('PURCHASE')} style={s.toggle(formType === 'PURCHASE', Colors.warning)}>Purchase Order</button>
        </div>
        <Field label="Party">
        ...
        <div style={{ display: 'flex', gap: Spacing.sm, marginTop: Spacing.md }}>
          <button onClick={handleSave} disabled={!partyId || lines.length === 0} style={!partyId || lines.length === 0 ? s.primaryBtnDisabled : s.primaryBtn}>Save Order</button>
        </div>
  ...
  const orderStatusColor = (st?: string) => {
    const map: Record<string, string> = { OPEN: Colors.info, CONFIRMED: Colors.success, CONVERTED: Colors.textDisabled, CANCELLED: Colors.error }
    return map[st || ''] || Colors.textSecondary
  }
  ...
  return (
    <div style={{ padding: Spacing.lg, paddingBottom: 80 }}>
      ...
      <div style={{ position: 'relative', marginBottom: Spacing.md }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search orders..." style={{ ...s.searchBox, paddingLeft: 36 }} />
      </div>
      {filtered.length === 0 ? (
        ...
        filtered.map(o => (
          <div key={o.id} style={{ ...s.card, marginBottom: Spacing.sm }}>
            <div style={{ ...s.spaceBetween, marginBottom: Spacing.xs }}>
              <span style={{ fontWeight: 700, fontSize: 15, color: Colors.textPrimary }}>#{o.invoiceNo}</span>
              <span style={s.badge(orderStatusColor(o.orderStatus))}>{o.orderStatus || 'OPEN'}</span>
            </div>
            <div style={{ fontSize: 12, color: Colors.textSecondary, marginBottom: Spacing.sm }}>{o.partyName} · {formatDate(o.date)}</div>
            <div style={{ ...s.spaceBetween, marginBottom: Spacing.sm }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 17, color: Colors.textPrimary }}>{formatCurrency(o.grandTotal)}</span>
            </div>
            <div style={{ display: 'flex', gap: Spacing.xs }}>
              <button onClick={(e) => { e.stopPropagation(); onNavigate('invoice-view?id=' + o.id) }} style={{ flex: 1, padding: '8px', backgroundColor: Colors.surfaceVariant, border: 'none', borderRadius: BorderRadius.sm, fontSize: 12, fontWeight: 600, color: Colors.textPrimary, cursor: 'pointer' }}>View</button>
              {o.orderStatus !== 'CONVERTED' && o.orderStatus !== 'CANCELLED' && (
                <button onClick={(e) => { e.stopPropagation(); handleConvert(o) }} style={{ flex: 1, padding: '8px', backgroundColor: Colors.primary + '15', border: 'none', borderRadius: BorderRadius.sm, fontSize: 12, fontWeight: 600, color: Colors.primary, cursor: 'pointer' }}>Convert to Invoice</button>
              )}
              <button onClick={(e) => { e.stopPropagation(); handleDelete(o.id) }} style={{ padding: '8px', backgroundColor: Colors.errorLight, border: 'none', borderRadius: BorderRadius.sm, color: Colors.error, cursor: 'pointer', display: 'flex', alignItems: 'center' }}><Icons.Delete size={16} /></button>
            </div>
          </div>
        ))
      )}
