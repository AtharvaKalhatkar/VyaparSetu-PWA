...
function calcDepreciation(a: FixedAsset): { yearlyDep: number; currentValue: number } {
  if (a.usefulLife <= 0) return { yearlyDep: 0, currentValue: a.purchasePrice }
  if (a.depreciationMethod === 'SLM') {
    ...
    for (let y = 0; y < fullYears; y++) value = value * (1 - rate)
    value = value * Math.pow(1 - rate, partialYear)
...
export function FixedAssets() {
  ...
  const handleSave = () => {
    ...
    const pp = parseFloat(purPrice) || 0; const dr = parseFloat(depRate) || 0; const ul = parseFloat(usefulLife) || 0
    const asset: FixedAsset = {
      id: editId || generateId(), name: name.trim(), category: category.trim() || 'General',
      purchaseDate: purDate, purchasePrice: pp,
      depreciationMethod: depMethod, depreciationRate: dr,
      usefulLife: ul, salvageValue: parseFloat(salvage) || 0, currentValue: pp,
