...
export function PosBilling({ onBack }: { onBack?: () => void }) {
  ...
  const handleSave = () => {
    ...
    const inv = {
      ...
      items: items.map(i => { const item = allItems.find(a => a.id === i.itemId); return { itemId: i.itemId, itemName: i.name, sku: item?.sku || '', quantity: i.qty, rate: i.rate, unit: item?.unit || '', discountPercent: 0, discountAmount: 0, gstRate: i.gst, amount: i.qty * i.rate } }),
    ...
    applyStockChanges(items.map(i => ({ itemId: i.itemId, quantity: i.qty, unit: allItems.find(a => a.id === i.itemId)?.unit || '' })), 'SALE')
          ...
          {items.map((item, idx) => (
            <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 6, backgroundColor: Colors.surface, borderRadius: BorderRadius.md, padding: '10px 12px', border: `1px solid ${Colors.border}` }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <input inputMode="decimal" value={item.qty} onChange={e => setQtyDirect(idx, e.target.value)} style={{ width: 40, textAlign: 'center', fontSize: 16, fontWeight: 700, color: Colors.textPrimary, border: `1px solid ${Colors.border}`, borderRadius: 6, padding: '2px 4px', background: Colors.surface }} />
                <button onClick={() => updateQty(idx, 1)} style={{ width: 36, height: 36, borderRadius: 8, border: `1px solid ${Colors.border}`, background: Colors.surfaceVariant, cursor: 'pointer', fontSize: 20, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
              </div>
            </div>
          ))}
