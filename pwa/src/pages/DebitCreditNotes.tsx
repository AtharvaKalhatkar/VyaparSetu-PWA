...
const NOTES_KEY = () => { const cid = getActiveCompanyId(); return cid ? `vs_${cid}_debitCreditNotes` : 'vs_debitCreditNotes' }
...
export function DebitCreditNotes() {
  ...
  const addNote = () => {
    ...
    if (!invoiceNo.trim()) { alert('Please enter a reference invoice'); return }
    if (isNaN(amt) || amt <= 0) { alert('Please enter a valid amount'); return }
    if (!reason.trim()) { alert('Please provide a reason'); return }
    const note: Note = {
      id: generateId(), type, partyId, invoiceNo: invoiceNo.trim(),
      amount: amt, reason: reason.trim(), date, status: 'UNAPPLIED',
    }
