...
export function SubscriptionsPage({ onNavigate }: { onNavigate: (p: string) => void }) {
  ...
  const generateInvoice = (sub: Subscription) => {
    ...
    const invItems = sub.items.map(i => {
      const item = allItems.find(x => x.id === i.itemId)
      const gstRate = item?.gstRate || 0
      const amount = i.quantity * i.rate
      return { itemId: i.itemId, itemName: i.itemName, sku: '', quantity: i.quantity, rate: i.rate,
        unit: i.unit, discountPercent: 0, discountAmount: 0, gstRate, amount,
      }
    })
    const subTotal = invItems.reduce((s, i) => s + i.amount, 0)
    const taxAmount = invItems.reduce((s, i) => s + i.amount * i.gstRate / 100, 0)
    const grandTotal = subTotal + taxAmount
    const inv = {
      id: generateId(),
      invoiceNo: nextInvoiceNo(DB.invoices.list().map(x => x.invoiceNo), 'INV'),
      partyId: sub.partyId, partyName: sub.partyName, type: 'SALE' as const, docType: 'SALE' as const,
      items: invItems, subtotal: subTotal, discountAmount: 0, taxAmount, grandTotal,
      paymentStatus: 'PENDING' as const, paidAmount: 0, dueAmount: grandTotal,
