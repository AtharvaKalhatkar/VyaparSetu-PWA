  ...
  const handleSave = () => {
    ...
    const inv: Invoice = {
      ...
      items: parsedItems.map(p => ({
        itemId: inventoryItems.find(i => i.name === p.name)?.id || (() => { const newItem: Item = { id: generateId(), name: p.name, unit: 'Pcs', sellingPrice: p.rate, purchasePrice: p.rate, currentStock: p.qty, minStockLevel: 0, isActive: true, sku: '', gstRate: 0 }; DB.items.save(newItem); return newItem.id })(),
