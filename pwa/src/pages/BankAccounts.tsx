...
export function BankAccounts() {
  ...
  const handleSaveTxn = () => {
    ...
    if (!txnAmount || !txnDesc.trim() || !expandedId || isNaN(amt) || amt <= 0) return
  ...
  const handleDeleteAccount = (id: string) => {
    const all = DB.bankTransactions.list().filter(t => t.accountId !== id)
    try { localStorage.setItem('vs_bankTxns', JSON.stringify(all)) } catch (e) { console.error('failed to save bank txns', e) }
