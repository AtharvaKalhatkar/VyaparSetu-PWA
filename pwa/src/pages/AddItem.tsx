...
export function AddItem({ editId, onBack, onNavigate, onAddUnit }: { editId?: string; onBack: () => void; onNavigate?: (p: string) => void; onAddUnit?: () => void }) {
  ...
  const handleSave = () => {
    if (expDate && mfgDate && expDate < mfgDate) { alert('Expiry date cannot be before manufacturing date'); return }
    if (sellingPrice && purchasePrice && parseFloat(purchasePrice) > parseFloat(sellingPrice)) { alert('Purchase price should not exceed selling price'); return }
    DB.items.save({
  ...
  return (
    <div style={{ padding: Spacing.lg, paddingBottom: 80 }}>
      ...
      {step === 2 && (
        <div>
          <Field label="Current Stock"><input type="number" min="0" value={currentStock} onChange={e => setCurrentStock(e.target.value)} placeholder="0" style={s.input} /></Field>
          <Field label="Minimum Stock Alert"><input type="number" min="0" value={minStock} onChange={e => setMinStock(e.target.value)} placeholder="10" style={s.input} /></Field>
      ...
      {step === 3 && config.enableGst && (
        <div>
          {name.length > 0 && (() => {
            const ai = suggestHsnAndGst(name)
            if (!ai.hsn) return null
            return (
              <div style={{ backgroundColor: Colors.infoLight, borderRadius: BorderRadius.md, padding: Spacing.md, marginBottom: Spacing.md, border: `1px solid ${Colors.info}30`, fontSize: 12 }}>
                <div style={{ fontWeight: 600, color: Colors.info, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
                  <Icons.Star size={14} /> AI Suggestion
                </div>
                <div style={{ color: Colors.textSecondary }}>Based on the name "<strong>{name}</strong>", we suggest:</div>
                <div style={{ display: 'flex', gap: Spacing.md, marginTop: Spacing.sm }}>
                  <button onClick={() => { setHsnCode(ai.hsn); setGstRate(String(ai.gstRate)) }} style={{ padding: '6px 14px', backgroundColor: Colors.primary, color: '#fff', border: 'none', borderRadius: BorderRadius.sm, fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Icons.Check size={14} /> HSN: {ai.hsn} · GST: {ai.gstRate}% · {ai.category}
                  </button>
                </div>
              </div>
            )
