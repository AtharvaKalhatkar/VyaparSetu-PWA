...
const TDS_KEY = () => { const cid = getActiveCompanyId(); return cid ? `vs_${cid}_tdsTcsEntries` : 'vs_tdsTcsEntries' }
function loadTds(): TdsTcsEntry[] {
  try { return JSON.parse(localStorage.getItem(TDS_KEY()) || '[]') } catch { return [] }
}
function saveTds(entries: TdsTcsEntry[]) { localStorage.setItem(TDS_KEY(), JSON.stringify(entries)) }

export function TdsTcs() {
  const [activeTab, setActiveTab] = useState<'TDS' | 'TCS'>('TDS')
  const [entries, setEntries] = useState<TdsTcsEntry[]>(() => loadTds())
