...
import { AiOfficeTeam } from './pages/AiOfficeTeam'
...
function setDarkMode(v: boolean) {
  ...
  catch (e) { console.error('dark mode write failed', e) }
}
...
export default function App() {
  ...
  const toggleDarkMode = (v: boolean) => { setDarkModeState(v); try { if (v) localStorage.setItem(DM_KEY, 'true'); else localStorage.removeItem(DM_KEY) } catch (e) { console.error('dark mode write failed', e) } }
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'ai-office-team': return <AiOfficeTeam navigate={navigate} />
