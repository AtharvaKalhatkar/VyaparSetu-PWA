...
export function useDefaultShareOption() {
  const [defaultOption, setDefaultOption] = React.useState(() => {
    try { return localStorage.getItem('vs_defaultShareOption') || 'pdf' } catch { return 'pdf' }
  })
  const saveDefault = (key: string) => {
    try { localStorage.setItem('vs_defaultShareOption', key) } catch (e) { console.error('failed to save default share option', e) }
