...
export function AddPayment({ onBack, onNavigate, invoiceId: propInvId }: { onBack: () => void; onNavigate?: (p: string) => void; invoiceId?: string }) {
  const [invId, setInvId] = useState(propInvId || '')
  const [amountStr, setAmountStr] = useState('')
  const [mode, setMode] = useState('CASH')
  const [saved, setSaved] = useState(false)

  const invoices = DB.invoices.list().filter(i => (i.docType === 'SALE' || i.type === 'SALE') && i.dueAmount > 0)
  ...
  const handleSave = () => {
    ...
    createLedgerEntry(inv.partyId, inv.partyName, 'RECEIPT', amt, mode, inv.invoiceNo, `Payment received for ${inv.invoiceNo}`, todayISO())
  ...
  return (
    <div style={{ padding: Spacing.lg, paddingBottom: 80 }}>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: Colors.textPrimary, marginBottom: Spacing.lg }}>Receive Payment</h2>
      ...
      {inv && (
        <div style={{ ...s.card, marginBottom: Spacing.md, backgroundColor: Colors.primaryLight }}>
          ...
          <div style={s.spaceBetween}><span style={{ fontSize: 13, color: Colors.textSecondary }}>Party</span><span style={{ fontWeight: 600 }}>{inv.partyName}</span></div>
