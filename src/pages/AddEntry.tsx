...
export function AddEntry({ onBack, onNavigate }: { onBack: () => void; onNavigate?: (p: string) => void }) {
  ...
  const handleSave = () => {
    ...
    if (type === 'EXPENSE') {
      DB.expenses.save({ id: generateId(), category, amount: parseFloat(amount), description: description.trim(), date: todayISO(), paymentMode: 'CASH' })
    } else {
      DB.ledger.save({ id: generateId(), partyId: '', partyName: 'Income', type: 'RECEIPT', amount: parseFloat(amount), mode: 'CASH', reference: '', description: description.trim(), date: todayISO(), runningBalance: 0 })
    }
    setSaved(true)
