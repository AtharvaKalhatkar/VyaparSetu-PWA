...
import { deleteInvoiceWithReversal, applyStockChanges } from '../utils/invoiceOps'
...
export function ReturnsPage({ onNavigate, sourceId }: { onNavigate: (p: string) => void; sourceId?: string }) {
  ...
  const handleSave = () => {
    ...
    DB.invoices.save({
      id: newId, docType: dt, invoiceNo: nextInvoiceNo(DB.invoices.list().filter(i => i.docType === dt).map(i => i.invoiceNo), prefix),
