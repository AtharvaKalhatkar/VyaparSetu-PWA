...
export function Billing({ editId, initialType, onBack, onNavigate }: { editId?: string; initialType?: 'SALE' | 'PURCHASE'; onBack?: () => void; onNavigate?: (p: string) => void }) {
  ...
  const saveInvoice = () => {
    ...
    // Apply new stock changes (batched to avoid stale reads)
    const newItems = lines.map(l => ({ itemId: l.itemId, quantity: safeNum(l.qty), unit: l.unit }))
    applyStockChanges(newItems, type)
    // Remove old ledger entry for this invoice if editing, then create new one
    DB.ledger.list().filter(l => l.reference === inv.invoiceNo).forEach(l => DB.ledger.delete(l.id))
    createLedgerEntry(
      partyId, party?.name || '',
      type === 'PURCHASE' ? 'PURCHASE' : 'SALE',
      grandTotal, 'CREDIT', inv.invoiceNo,
      existing ? 'Invoice updated' : type === 'PURCHASE' ? 'Purchase invoice' : 'Sale invoice',
      date
    )
    setSaved(true)
