...
export function BalanceSheet() {
  ...
  const filtered = period === 'current' ? invoices
    : period === 'month' ? invoices.filter(i => i.date >= monthStart && i.date <= today)
    : invoices.filter(i => i.date >= yearStart && i.date <= today)

  const bankBalance = accounts.reduce((s, a) => s + a.balance, 0)
  const receivables = filtered.filter(i => i.type === 'SALE').reduce((s, i) => s + i.dueAmount, 0)
  const payables = filtered.filter(i => i.type === 'PURCHASE').reduce((s, i) => s + i.dueAmount, 0)
