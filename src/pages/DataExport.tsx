...
export function DataExport() {
  ...
  const handleRestore = () => {
    ...
    const d = importData
    const save = (key: string, data: any) => localStorage.setItem('vs_' + key, JSON.stringify(data))
  ...
  const lastBackup = localStorage.getItem('vs_lastBackup')
  const backupTime = lastBackup ? Math.floor((Date.now() - parseInt(lastBackup)) / 3600000) : null

  const handleBackupNow = () => {
    handleExportAll()
    localStorage.setItem('vs_lastBackup', String(Date.now()))
  }
