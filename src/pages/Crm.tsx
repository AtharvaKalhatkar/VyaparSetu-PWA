...
export function Crm() {
  ...
  const openEdit = (id: string) => {
    ...
    if (!l) return
    setEditId(id); setName(l.name); setPhone(l.phone); setEstValue(String(l.estimatedValue)); setShowForm(true)
  }

  const handleSave = () => {
    if (!name.trim()) return
    DB.crm.save({ id: editId || generateId(), name: name.trim(), phone: phone.trim(), source: 'OTHER', status: 'NEW', estimatedValue: parseFloat(estValue) || 0, createdAt: new Date().toISOString().split('T')[0] })
    setShowForm(false); setEditId(null); setName(''); setPhone(''); setEstValue('')
  }
