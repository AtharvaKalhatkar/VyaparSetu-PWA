...
export default function SuppliersScreen() {
  ...
  const fetchParties = useCallback(async () => {
    try {
      const data = await ApiService.party.getParties({ limit: 100, type: 'SUPPLIER', sort: 'name', order: 'asc' });
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => router.push('/add-party')}><Icon name="truck-plus" size={24} color={Colors.primary} /></TouchableOpacity>
      </View>
      <Searchbar placeholder="Search suppliers..." value={search} onChangeText={setSearch} style={styles.searchBar} />
      {loading ? <View style={styles.center}><ActivityIndicator size="large" color={Colors.primary} /></View> : (
        <FlatList data={filtered} keyExtractor={item => item.id}
          ...
          ListEmptyComponent={<EmptyState icon="truck" title="No suppliers" message={search ? 'Try a different search' : 'Add your first supplier'} actionLabel="Add Supplier" onAction={() => router.push('/add-party')} />}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: Colors.warning + '20', justifyContent: 'center', alignItems: 'center', marginRight: Spacing.md },
  avatarText: { fontSize: 18, fontWeight: '700', color: Colors.warning },
  info: { flex: 1 },
